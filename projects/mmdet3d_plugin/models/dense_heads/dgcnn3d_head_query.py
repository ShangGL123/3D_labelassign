...
class DGCNN3DHead_Query(DETRHead):
    ...
    def __init__(self,
        ...
        if self.as_two_stage:
            ...
            print(transformer['as_two_stage'])
        ...
        super(DGCNN3DHead_Query, self).__init__(
    ...
    def _init_layers(self):
        ...
        semantic_branch.append(Linear(self.embed_dims, self.cls_out_channels))
        self.fc_sem = nn.Sequential(*semantic_branch)
    ...
    def forward(self, mlvl_feats, querys):
        """Forward function.
        Args:
        ...
        # print('input_img_h, input_img_w',input_img_h, input_img_w)
        # print('feature_shape',mlvl_feats[0].shape[-2:])
        ...
        if not self.as_two_stage:
            ...
            # query_embeds = query_embeds.unsqueeze(0).expand(1, -1, -1)
            # query_pos1, query1 = torch.split(query_embeds, 256, dim=-1)
            
            # print(type(querys))
            # query, pos = querys['sa_xyz'][-1], querys['sa_features'][-1]
            query_pos, query = torch.split(querys, [3, 256] , dim=-1)
            #进行分割
            point_cls_semantic_preds = self.fc_sem(query)

            query_pos_tmp = query_pos[..., 0:2]  # 舍去z
            query_embeds = torch.cat([query_pos_tmp, query], dim=-1)
            # query_embeds = querys
        ...
        if self.as_two_stage:
            ...
            outs = {
                ...
                'enc_cls_scores': None,
                'enc_bbox_preds': None, 
                'point_cls_semantic_preds' : point_cls_semantic_preds,
            }
        return outs
    ...
    def loss(self,
        ...
        point_cls_semantic_preds = preds_dicts['point_cls_semantic_preds']
        print(gt_bboxes_list[0][0])
