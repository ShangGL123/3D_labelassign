...
class CenterHead_cost_assign_1task_gfl(BaseModule):
    ...
    def __init__(self,
        ...
        super(CenterHead_cost_assign_1task_gfl, self).__init__(init_cfg=init_cfg)
    ...
    def get_targets_single(self, gt_bboxes_3d, gt_labels_3d, preds_dicts_a_batch):
        ...
        #解码到3d lidar格式 rot = torch.atan2(rot_sine, rot_cosine)  torch.exp(preds_dict[0]['dim'])
        ...
        score = gt_bboxes_3d.new_zeros(labels.shape)
        score[pos_inds] = all_ious[pos_inds, assigned_gt_inds[pos_inds] - 1]

        loss_cls = self.loss_cls(cls_pred, (labels, score), avg_factor=1.)     # 这里avg_factor要修改
